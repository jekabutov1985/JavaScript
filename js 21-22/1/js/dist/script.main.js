...
$(function() {
	...
	if($('input:not(:checked)')) {
		var error = 'Не хотите отвечать на вопросы?. ' + '<hr>';
	...
	function showResult(){
		...
		for(var k=0; k< $chBox.length; k++) {
			...
			if (QBase.question[i].answer[j].check){
				result = 'Вариант № ' + (j+1) + ' ' + 'на вопрос № ' + (i+1) + ' ' + '<span class="correct">' + 'верный. ' + '</span>' + '<hr>';
				$('#modalText').append(result);
			} else {
				result = 'Вариант № ' + (j+1) + ' ' + 'на вопрос № ' + (i+1) + ' ' + '<span class="incorrect">' + 'неверный. ' + '</span>' + '<hr>';
				$('#modalText').append(result);
			}
		}
		var selection = $('input');
